...
if use_nvcc:
    ...
    link_args = ["-shared", "-g", "-O2", "-D_FORTIFY_SOURCE=2"]
else:
    compile_args = []
    link_args = []
...
class nvcc_build_ext(build_ext):
    def build_extensions(self):
        self.compiler.set_executable(
            "compiler_so", f"/opt/rocm-{ROCM_VERSION}/hip/bin/hipcc"
        )
        self.compiler.set_executable(
            "compiler_cxx", f"/opt/rocm-{ROCM_VERSION}/hip/bin/hipcc"
        )
        self.compiler.set_executable(
            "linker_so", f"/opt/rocm-{ROCM_VERSION}/hip/bin/hipcc"
        )
...
os.environ["CC"] = f"/opt/rocm-{ROCM_VERSION}/hip/bin/hipcc"
os.environ["CXX"] = f"/opt/rocm-{ROCM_VERSION}/hip/bin/hipcc"
